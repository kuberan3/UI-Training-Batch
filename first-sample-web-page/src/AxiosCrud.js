...
const Axioscrud = () => {
  ...
  const handleUpdate = (data) => {
    ...
    axios.put(`http://localhost:8000/data/${data.id}`, {
      brand: `${productName}`,
      price: `${data.price}`,
      rating: `${data.rating}`,
  ...
  const handleadd = () => {
    ...
    if (BrandExists) {
      ...
      axios
        .post("http://localhost:8000/data", toAdd)
  ...
  return (
    <MainDiv>
      <AddDiv>
        ...
        <br />
        <label>Rating :</label>
        <input id="inputBox" name="rating" type="number" max={5} min={1} required onChange={handleAddChange}></input>
        <br />
        <label>Price :</label>
        ...
        {data?.map((product, index) => {
          return (
            <ProductDiv>
              ...
              <StyledButton onClick={() => handleDelete(product.id)}>
                Delete Product
              </StyledButton>
            </ProductDiv>
          );
